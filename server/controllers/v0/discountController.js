...
import {
  V0_DISCOUNT_CREATE_ENDPOINT,
  V0_DISCOUNT_UPDATE_ENDPOINT,
  V0_DISCOUNT_DELETE_ENDPOINT,
...
export async function create(request, response) {
  ...
  try {
    ...
    logger.info(error);
    logger.info(`Error posting a new discount: ${JSON.stringify(postBody)}`);
    return response.status(400).send('Failed to create a new discount');
  }
}
...
export async function update(request, response) {
  ...
  try {
    ...
    logger.info(error);
    logger.info(`Error updating a discount: ${JSON.stringify(postBody)}`);
    return response.status(400).send('Failed to update a discount');
  }
}
...
export async function deleteResource(request, response) {
  ...
  try {
    ...
    logger.info(error);
    logger.info(`Error deleting a discount: ${JSON.stringify(postBody)}`);
    return response.status(400).send('Failed to delete a discount');
  }
}
