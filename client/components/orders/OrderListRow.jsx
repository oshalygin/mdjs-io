...
const OrderListRow = ({ order }) => {
  ...
  const statusColor = order.orderStatusID < 100 ? '#5CB85C' : '#F44336';

  return (
    <div
      styleName="list-row-container"
      style={{
        fontWeight: order.expanded ? 'bold' : 'normal',
      }}
    >
      <div styleName="first-section">
        {order.orderID}
      </div>
      <div styleName="second-section">
        {parsedCreatedDate}
      </div>
      <div styleName="third-section" style={{ color: '#F44336' }}>
        {!!order.totalDiscount &&
          `( ${numberToLocaleString(order.totalDiscount)})`}
      </div>
      <div styleName="fourth-section">
        {numberToLocaleString(order.totalTax)}
      </div>
      <div styleName="fifth-section">
        {numberToLocaleString(order.total)}
      </div>
      <div styleName="sixth-section" style={{ color: statusColor }}>
