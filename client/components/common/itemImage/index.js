...
export function getImageStyle(imageId, color) {
  const itemColor = color
    ? itemColors.find(itemColorEntity => itemColorEntity.value === color).color
    : '#778088';

  const backgroundColor = itemColor;

  const backgroundImage = imageId ? `url(${IMAGE_ENDPOINT}/${imageId})` : '';
...
const ItemImage = ({ itemId, color, imageId, label, onClick }) => {
  ...
  return (
    <div styleName="image-container" onClick={() => onClick(itemId)}>
