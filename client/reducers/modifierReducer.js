...
export default function modifierReducer(
  state = initialState.modifiers,
  action,
) {
  switch (action.type) {
    ...
    case actionTypes.MODIFIER_UPDATED: {
      ...
      const currentModifierIndex = modifiers.findIndex(
        modifier => modifier.modifierID === action.modifier.modifierID,
      );
